...
local appId = "861105243492581416"
...
function checkArg(arg, argType, argName, func, maybeNil)
    assert(type(arg) == argType or (maybeNil and arg == nil), string.format("Argument \"%s\" to function \"%s\" has to be of type \"%s\"", argName, func, argType))
end

function checkStrArg(arg, maxLen, argName, func, maybeNil)
    if maxLen then
        assert(type(arg) == "string" and arg:len() <= maxLen or (maybeNil and arg == nil), string.format("Argument \"%s\" of function \"%s\" has to be of type string with maximum length %d", argName, func, maxLen))
    else
...
function checkIntArg(arg, maxBits, argName, func, maybeNil)
    ...
    assert(type(arg) == "number" and math.floor(arg) == arg and arg < maxVal and arg >= -maxVal or (maybeNil and arg == nil), string.format("Argument \"%s\" of function \"%s\" has to be a whole number <= %d", argName, func, maxVal))
end
...
presence = {
    ...
    state = "Workspace: PT Cinta Abadi",
    largeImageKey = "large",
    largeImageText = "Editing a ASS file",
    smallImageKey = "small",
    smallImageText = "Aegisub",
    startTimestamp = now
}
